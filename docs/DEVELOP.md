...
公開したら、`pages.yml`の`default`を公開した版のコミットに書きかえてコミットします(次に動かすときの既定になり、いまどの版を公開しているかも分かる)。いまの`default`は、ステージ1から3とフリープレイ、ボス戦の連打の技、煙とUFOの光の粒、称号の直し、テストの整理、おばあちゃんの敵の条件の文の直し、絵の全面の描き直し、会話の窓の48×48の顔に、ステージ4(高層ビル)と、ステージ3と4の文の見直しを足した`01a3fd6`です。
