...
//   古くなるほど渦に流されてうねる。粒は3ドット、2ドット、1ドットと縮み、最後は1コマおきに点滅して消える。
// HermiteSparks:光の粒が from から to へ吸いこまれる。道すじはエルミート曲線(flow.ts の hermite)で、
//   横へふくらんで出て、to にはまっすぐ上から入る。粒ごとに出る向きを変えるので、らせんを巻くように見える。
...
/** 黒い煙の色(出たばかり → 古い)。R、G、B は8段階の色から選ぶ。薄まるほど明るい灰色にして、暗い背景でも見えるようにする */
export const SMOKE_DARK = [0x242424, 0x494949, 0x6d6d6d, 0x929292] as const;
...
export class CurlSmoke extends FlowEmitter {
  ...
  protected step(dt: number, time: number): number {
    ...
    for (const d of this.dots) {
      ...
      const size = d.ember ? 1 : p < 0.3 ? 3 : p < 0.65 ? 2 : 1;
      const h = Math.floor(size / 2);
      g.fillStyle(c, 1).fillRect(Math.round(d.x) - h, Math.round(d.y) - h, size, size);
    }
    this.dots = keep;
    return keep.length;
  }
